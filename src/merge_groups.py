...
def merge_groups(high_dim_data, init_groups, max_groups, method_distance="cityblock", method_clust="complete"):
    """
    ...
    if high_dim_data is None or init_groups is None or max_groups is None:
        raise ValueError("Please provide valid high_dim_data, init_groups, and max_groups.")
    
    if max_groups < 2:
        raise ValueError("The number of new groups must be at least 2.")
    
    if len(init_groups) < max_groups:
        raise ValueError("The number of new groups cannot be greater than the number of initial groups.")
    ...
    for i in range(max_groups):
        ...
        group_new[i] = list(set([item for sublist in [init_groups[clust_names[j]] for j in id_clust_i] for item in sublist]))
