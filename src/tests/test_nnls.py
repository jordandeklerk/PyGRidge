...
def test_nonneg_lsq_multiple_rhs(random_A_B):
    A, B = random_A_B
    X = nonneg_lsq(A, B, alg='fnnls')
    assert X.shape == (A.shape[1], B.shape[1]), "Solution shape mismatch for multiple RHS."
...
def test_nonneg_lsq_correctness(simple_A_B):
    ...
    assert np.isclose(residual, scipy_residual, atol=1e-6), "Residuals do not match SciPy's nnls."
...
def test_nonneg_lsq_scipy_comparison(random_A_B):
    ...
    for i in range(B.shape[1]):
        ...
        np.testing.assert_allclose(X[:, i], scipy_X, atol=1e-6, err_msg=f"Mismatch in column {i}")
...
def test_nonneg_lsq_gram_matrix(random_A_B):
    ...
    for i in range(B.shape[1]):
        scipy_X, scipy_res = scipy_nnls(A, B[:, i])
        np.testing.assert_allclose(X[:, i], scipy_X, atol=1e-6, err_msg=f"Mismatch in Gram matrix column {i}")
...
def test_nonneg_lsq_parallel():
    ...
    np.testing.assert_allclose(X_parallel, X_sequential, atol=1e-6, err_msg="Parallel and sequential solutions differ.")
...
def test_nonneg_lsq_invalid_algorithm(simple_A_B):
    A, B = simple_A_B
    with pytest.raises(ValueError, match="Specified algorithm 'invalid_alg' not recognized."):
...
def test_nonneg_lsq_empty_matrix():
    A = np.empty((0, 0))
    B = np.empty((0,))
    with pytest.raises(InvalidInputError, match="Input matrices A and B must not be empty."):
...
def test_nonneg_lsq_gram_incompatible_shapes():
    A = np.random.rand(5, 3)
    B = np.random.rand(4)
    with pytest.raises(InvalidInputError, match="When gram=True, A must be a square matrix."):
...
def test_nonneg_lsq_single_variable():
    ...
    np.testing.assert_allclose(X, expected, atol=1e-6, err_msg="Single variable solution incorrect.")
...
def test_nonneg_lsq_large_problem():
    ...
    assert np.isclose(residual, scipy_res, atol=1e-4), "Residuals do not match SciPy's nnls in large problem."
...
def test_nonneg_lsq_max_iter(simple_A_B):
    ...
    with pytest.raises(ConvergenceError, match="FNNLS failed to converge"):
        nonneg_lsq(A, B, alg='fnnls', max_iter=0)  # Set max_iter=0 to force convergence failure

def test_nonneg_lsq_non_unique_solution():
    A = np.array([[1, 1], [2, 2], [3, 3]], dtype=float)
    B = np.array([6, 12, 18], dtype=float)
    X = nonneg_lsq(A, B, alg='fnnls')
    assert np.isclose(X[0] + X[1], 6, atol=1e-6), "Non-unique solution does not satisfy sum constraint."
...
def test_nonneg_lsq_no_solution():
    ...
    np.testing.assert_allclose(X, expected, atol=1e-6, err_msg="Negative B should lead to zero solution.")
...
def test_nonneg_lsq_sparse_matrix():
    ...
    np.testing.assert_allclose(X, expected, atol=1e-6, err_msg="Sparse matrix solution incorrect.")
...
def test_nonneg_lsq_precision():
    ...
    np.testing.assert_allclose(X, expected, atol=1e-12, err_msg="Precision test failed.")
...
def test_nonneg_lsq_highly_correlated_columns():
    ...
    np.testing.assert_allclose(X, expected, atol=1e-3, err_msg="Highly correlated columns solution incorrect.")
...
def test_fnnls_core_individual_solution():
    ...
    np.testing.assert_allclose(X, expected, atol=1e-6, err_msg="fnnls_core individual solution incorrect.")
...
def test_fnnls_core_no_positive_gradients():
    ...
    np.testing.assert_allclose(X, expected, atol=1e-6, err_msg="fnnls_core should return zero vector.")
...
def test_fnnls_core_all_positive():
    ...
    X = fnnls_core(AtA, Atb)
    expected = np.array([1., 1.])
    np.testing.assert_allclose(X, expected, atol=1e-6, err_msg="fnnls_core should return exact solution when all variables are positive.")
