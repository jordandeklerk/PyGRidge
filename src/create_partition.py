...
def create_variable_groups(
    ...
    if values.dtype.kind in "OSU":  # Object, String, or Unicode type
        ...
        if uniform_groups or num_groups * min_group_size >= len(values):
            ...
            sorted_indices = (
                np.argsort(values)[::-1] if sort_descending else np.argsort(values)
            )
            total_elements = len(values)

            if sort_descending:
                print(
                    "Sorting values in descending order, assuming small values are LESS"
                    " relevant"
                )
            else:
                print(
                    "Sorting values in ascending order, assuming small values are MORE"
                    " relevant"
                )
            ...
            groups = {
                f"group{i+1}": (
                    sorted_indices[i * group_size : (i + 1) * group_size].tolist()
                    if i < actual_num_groups - 1
                    else sorted_indices[i * group_size :].tolist()
                )
            ...
            group_sizes = np.array(
                [
                    (
                        np.floor(min_group_size * root**i)
                        if i == 1
                        else np.round(min_group_size * root**i)
                    )
            ...
            print(
                f"Min: {np.min(group_sizes)}, Max: {np.max(group_sizes)}, Mean:"
                f" {np.mean(group_sizes):.2f}, Median: {np.median(group_sizes)}"
            )

            cumulative_sizes = np.cumsum(np.concatenate(([0], group_sizes))).astype(int)
            groups = {
                f"group{i+1}": sorted_indices[
    ...
    if not np.issubdtype(values.dtype, np.character) and subset is not None:
        ...
        subset_indices = [
            variable_names.index(s) if s in variable_names else None for s in subset
        ]
