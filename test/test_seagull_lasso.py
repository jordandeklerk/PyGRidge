...
def test_seagull_lasso_zero_lambda(sample_data):
    ...
    assert np.all(result['iterations'] == 1)  # Should converge in one iteration for lambda = 0
...
def test_seagull_lasso_large_lambda(sample_data):
    ...
    result = seagull_lasso(**sample_data)
    assert np.all(np.abs(result['random_effects']) < 1e-6)  # All coefficients should be close to zero
