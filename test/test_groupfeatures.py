...
class TestGroupedFeatures:
    ...
    @pytest.mark.parametrize(
        "group_size, num_groups, expected_exception, message",
        [
    ...
    @pytest.mark.parametrize(
        "ps, i, expected_exception, message",
        [
    ...
    @pytest.mark.parametrize(
        "ps, vec, f, expected_exception, message",
        [
            (
                [2, 3],
                "not a list",
                sum_func,
                TypeError,
                "vec must be either a list or ndarray, got str",
            ),
            (
                [2, 3],
                [1, 2],
                sum_func,
                ValueError,
                "Length of vec \\(2\\) does not match number of features \\(5\\)",
            ),
            (
                [2, 3],
                [1, 2, 3, 4, 5],
    ...
    def test_group_summary_function_exception(self):
        ...
        with pytest.raises(
            RuntimeError,
            match="Error applying function to group 0-2: Invalid function for testing",
        ):
    ...
    @pytest.mark.parametrize(
        "ps, vec_or_num, expected_exception, message",
        [
            (
                [2, 3],
                [1],
                ValueError,
                r"Length of vec_or_num \(1\) does not match number of groups \(2\)",
            ),
            (
                [2, 3],
                "not a number or list",
                TypeError,
                r"vec_or_num must be either a number",
            ),
    ...
    @pytest.mark.parametrize(
        "value, length, expected_exception, message",
        [
