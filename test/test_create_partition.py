...
def test_decreasing_order():
    vec = [5, 2, 8, 1, 9]
    result = create_variable_groups(
        vec, sort_descending=True, uniform_groups=True, group_size=1
    )
...
def test_increasing_order():
    vec = [5, 2, 8, 1, 9]
    result = create_variable_groups(
        vec, sort_descending=False, uniform_groups=True, group_size=1
    )
...
def test_non_uniform_partition():
    vec = list(range(100))
    result = create_variable_groups(
        vec, uniform_groups=False, num_groups=5, min_group_size=10
    )
    assert len(result) == 5
    assert all(len(group) >= 10 for group in result.values())
