...
def test_numerical_stability():
    ...
    result = lambda_max_group_lasso(y, groups, feature_weights, beta, X)
    assert np.isfinite(result), "Lambda max should be finite even with very small weights"

def test_input_validation():
    # Test with mismatched input sizes
    result = lambda_max_group_lasso(np.array([1, 2]), np.array([1, 1]), np.array([1, 1]), np.array([0, 0]), np.eye(2))
    assert result > 0, "Function should handle mismatched input sizes gracefully"

    # Test with mismatched beta size
    result = lambda_max_group_lasso(np.array([1, 2]), np.array([1, 1]), np.array([1, 1]), np.array([0]), np.eye(2))
...
def test_scale_invariance(sample_data):
    ...
    result2 = lambda_max_group_lasso(**sample_data)
    assert np.isclose(result1, result2 / (scale_factor**2), rtol=1e-5), f"Lambda max should scale quadratically with input. Got {result1} and {result2/(scale_factor**2)}"
